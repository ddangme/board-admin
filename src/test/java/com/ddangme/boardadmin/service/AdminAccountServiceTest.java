...
import com.ddangme.boardadmin.repository.AdminAccountRepository;
...
class AdminAccountServiceTest {
    ...
    void givenExistentUserId_whenSearching_thenReturnsOptionalUserData() {
        ...
        // Then
        assertThat(result).isPresent(); // Optional에 래핑되어 있는 값이 존재한다면 즉, empty가 아니라면 통과!
    ...
    void givenUserId_whenDeleting_thenDeletesAdminAccount() {
        // Given
        String userId = "uno";
        willDoNothing().given(adminAccountRepository).deleteById(userId); // willDoNothing()은 특정 메서드 호출 시 아무런 작업도 하지 않도록 설정하는 메소드이다.
